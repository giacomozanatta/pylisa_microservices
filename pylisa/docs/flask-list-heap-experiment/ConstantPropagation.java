...
public class ConstantPropagation
	...
	public ConstantPropagation evalBinaryExpression(
		...
		if (operator instanceof ComparisonEq || operator instanceof ComparisonNe) {
			// when both operands are concrete constants (numbers, strings, ...),
			// decide the comparison precisely; otherwise stay TOP. This is the
			// minimal addition needed by Flask's Route to read methods=[...]
			// list cells via Analysis.satisfies, which evaluates a value-level
			// equality check after the heap rewrite has resolved the cell to
			// its AllocationSite identifier.
			if (left.isTop() || right.isTop() || left.isBottom() || right.isBottom())
				return top();
			boolean equal = Objects.equals(left.constant.getValue(), right.constant.getValue());
			boolean truth = operator instanceof ComparisonEq ? equal : !equal;
			return new ConstantPropagation(new Constant(BoolType.INSTANCE, truth, pp.getLocation()));
		}
		return top();
	}
	...
	public Satisfiability satisfiesBinaryExpression(
		...
		if (operator instanceof ComparisonEq || operator instanceof ComparisonNe) {
			...
			boolean equal = Objects.equals(left.constant.getValue(), right.constant.getValue());
			boolean truth = operator instanceof ComparisonEq ? equal : !equal;
			return truth ? Satisfiability.SATISFIED : Satisfiability.NOT_SATISFIED;
		}
		return Satisfiability.UNKNOWN;
	}
